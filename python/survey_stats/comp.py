...
def load_all(lang_db, colls_db, fn_survey_input_en, fn_survey_input_pl):
    ...
    original_en = fm.load_csv(fn=fn_survey_input_en, survey_lang="en")
    logging.info(f"english group is available ({original_en.shape[0]} participants)")
    # (2) get means for english group
    means_en = target.get_means(
        original_en, colls_db=colls_db, lang_db=lang_db, survey_lang="en"
    )
    # (3) load polish csv file, translate it to english
    original_pl = fm.load_csv(fn=fn_survey_input_pl, survey_lang="pl")
    original_pl = lang.translate_pl_to_en(obj=original_pl, lang_db=lang_db)
    logging.info(f"polish group is available ({original_pl.shape[0]} participants)")
    # (4) get means for polish group
    means_pl = target.get_means(
        original_pl, colls_db=colls_db, lang_db=lang_db, survey_lang="pl"
    )
...
def remove_participants(
    ...
    list_wrong_en = target.find_wrong_answers(
        obj=means_en, d_filter_conditions=d_filter_conditions, survey_lang="en"
    )
    means_en = target.drop_rows(
        obj=means_en, target_list=list_wrong_en, survey_lang="en"
    )
    # (3) remove polish participants who did not answer with X to a question
    list_wrong_pl = target.find_wrong_answers(
        obj=means_pl, d_filter_conditions=d_filter_conditions, survey_lang="pl"
    )
    means_pl = target.drop_rows(
        obj=means_pl, target_list=list_wrong_pl, survey_lang="pl"
    )
    # (4) remove english participants who kept clicking the same answer
    list_clickers_en = target.find_clickers(
        obj_means=means_en,
    ...
    means_en = target.drop_rows(
        obj=means_en, target_list=list_clickers_en, survey_lang="en"
    )
    # (5) remove polish participants who kept clicking the same answer
    list_clickers_pl = target.find_clickers(
        obj_means=means_pl,
    ...
    means_pl = target.drop_rows(
        obj=means_pl, target_list=list_clickers_pl, survey_lang="pl"
    )
...
def get_clean_dfs(
    ...
    if enabled["use_csv_from_input"] is True:
        ...
        d_filter_conditions = {
            lang.lstr(lang_db, "is_l1", category="column"): lang.lstr(
                lang_db, "yes", category="answer"
            ),  # match only if polish is L1
            lang.lstr(lang_db, "is_l2", category="column"): lang.lstr(
                lang_db, "yes", category="answer"
            ),  # match only if english is L2
            lang.lstr(lang_db, "how_often", category="column"): lang.lstr(
                lang_db, "often_daily", category="answer"
            ),
        }  # match only if english is used daily
        means_en, means_pl = remove_participants(
            lang_db=lang_db,
            means_en=means_en,
        ...
        logging.info(
            f"processed english survey: {original_en.shape[0]} -> {means_en.shape[0]} participants"
        )
        logging.info(
            f"processed polish survey: {original_pl.shape[0]} -> {means_pl.shape[0]} participants"
        )
        # (III) save to means to csv in "./output"
        means_en = target.rename_index_to_participant(
            means_en
        )  # start numbering from 1, rename index to "Participant"
        means_pl = target.rename_index_to_participant(
            means_pl
        )  # start numbering from 1, rename index to "Participant"
    ...
    # otherwise, load csvs from output; this is useful if you want to edit them directly
        ...
        logging.info(
            "ok: loading processed csvs from output because 'use_csv_from_input' is False"
        )
        means_en = fm.load_csv(fn=fn_survey_output_en, survey_lang="en")
        means_pl = fm.load_csv(fn=fn_survey_output_pl, survey_lang="pl")
...
def get_participants_statistics(lang_db, means_en, means_pl, enabled):
    ...
    if enabled["get_participant_size"] is True:
        r.update(
            {
                "participant size": {
                    "english group": means_en.shape[0],
    ...
    if enabled["get_began_english"] is True:
        dict_learn_en = target.target_learn(
            means_en, lang.lstr(lang_db, "age_begin_eng", category="column")
        )
        dict_learn_pl = target.target_learn(
            means_pl, lang.lstr(lang_db, "age_begin_eng", category="column")
        )
        r.update(
            {"en: began to learn": dict_learn_en, "pl: began to learn": dict_learn_pl}
        )
    # (3) calculate age
    if enabled["get_age"] is True:
        # df_both = pd.concat([means_en, means_pl], axis=0, ignore_index=True)
        # print(df_both)
        dict_age_en = target.target_age(
            means_en, lang.lstr(lang_db, "birth_year", category="column")
        )
        dict_age_pl = target.target_age(
            means_pl, lang.lstr(lang_db, "birth_year", category="column")
        )
        r.update({"en: age": dict_age_en, "pl: age": dict_age_pl})
    # (4) calculate gender
    if enabled["get_gender"] is True:
        dict_gender_en = target.target_gender(
            means_en, lang.lstr(lang_db, "what_gender", category="column")
        )
        dict_gender_pl = target.target_gender(
            means_pl, lang.lstr(lang_db, "what_gender", category="column")
        )
        r.update({"en: gender": dict_gender_en, "pl: gender": dict_gender_pl})
    # (5) calculate city size
    if enabled["get_city"] is True:
        dict_city_en = target.target_city(
            means_en, lang.lstr(lang_db, "how_big_city", category="column")
        )
        dict_city_pl = target.target_city(
            means_pl, lang.lstr(lang_db, "how_big_city", category="column")
        )
        r.update({"en: city size": dict_city_en, "pl: city size": dict_city_pl})
    # (6) calculate uni year
    if enabled["get_uni_year"] is True:
        dict_uni_en = target.target_uni(
            means_en, lang.lstr(lang_db, "which_uni_year", category="column")
        )
        dict_uni_pl = target.target_uni(
            means_pl, lang.lstr(lang_db, "which_uni_year", category="column")
        )
        r.update({"en: uni year": dict_uni_en, "pl: uni year": dict_uni_pl})
