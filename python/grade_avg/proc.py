...
def load_html(filename, encoding="utf-8") -> str:
    """
...
def create_soup(html_content) -> BeautifulSoup:
    """
    Create bs4 soup object using html content.
    """
    try:
        soup: BeautifulSoup = BeautifulSoup(html_content, features="html.parser")
    except Exception as e:
        logging.error(f"failed to convert webpage to soup ({e})")
        raise
    else:
        logging.debug("ok: converted webpage to soup")
        return soup
...
def get_grades_from_table(table_content):
    ...
    for row in table_content.find_all("tr"):
        ...
        # if 4 columns then grade, get third column because it contains grade (counts from zero, so: 0, 1, 2, 3)
            ...
            if len(current_title) < 3:
                logging.error(
                    f"found grade but semester title is not available, skipping current grade: {current_title}"
                )
                continue
            # get text of subject
            ...
            try:
                ...
                if len(grade_list) > 1:
                    logging.debug(
                        f"multiple grades detected, trying to calculate average: {grade_list}"
                    )
                    avg = sum(temp) / len(temp)
                    logging.debug(
                        f"succeeded in getting average of {grade_list}, result is: {avg}"
                    )
                    subject = f"{subject} | average: {round(avg, 2)}"
            except Exception:
                logging.warning(
                    f"failed to convert to float, leaving as-is: {grade_list}"
                )
    ...
    if not total_avg_grade:
        logging.critical(
            "zero grades were extracted, quitting program; please check if titles of semesters are available"
        )
        quit()
...
def calculate_main(filepath_open, filepath_save):
    """
    Load grades from an offline copy of USOS.
    Save results to output.txt.
    """
    # load html from html file
    html = load_html(filepath_open)
    # create bs4 object
    ...
    save_dictionary_to_txt(grades_dict, avg_grade, filepath_save)
    return
